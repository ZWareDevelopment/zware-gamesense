...
public class OffhandGap extends Module {
	...
	private boolean shouldTotem(){
		final boolean hp = mc.player.getHealth() + mc.player.getAbsorptionAmount() <= health.getValue();
		final boolean endcrystal = !this.isCrystalsAABBEmpty();
		return hp;
	}

	private boolean isEmpty(final BlockPos pos){
		final List<Entity> crystalsInAABB = mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(pos)).stream().filter(e -> e instanceof EntityEnderCrystal).collect(Collectors.toList());
