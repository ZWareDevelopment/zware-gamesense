package dev.zihasz.zware.client.clickgui.frame;
...
import dev.zihasz.zware.client.clickgui.buttons.*;
...
public class Buttons extends Component {
	...
	public Buttons(final Module mod, final Frames parent, final int offset) {
		...
		this.open = false;
		int opY = offset + 16;
		if (ZWareMod.getInstance().settingsManager.getSettingsForMod(mod) != null && !ZWareMod.getInstance().settingsManager.getSettingsForMod(mod).isEmpty()) {
			for (final Setting s : ZWareMod.getInstance().settingsManager.getSettingsForMod(mod)) {
				switch (s.getType()) {
					case MODE:
