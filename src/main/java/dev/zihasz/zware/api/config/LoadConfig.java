...
public class LoadConfig {
    ...
    public void loadModuleDirect(String moduleLocation, Module module) throws IOException {
        ...
        for (Setting setting : ZWareMod.getInstance().settingsManager.getSettingsForMod(module)){
    ...
    public void loadCustomFont() throws IOException {
        ...
        if (name != null && size != -1){
            ZWareMod.fontRenderer = new CFontRenderer(new Font(name, Font.PLAIN, size), true, true);
            ZWareMod.fontRenderer.setFont(new Font(name, Font.PLAIN, size));
            ZWareMod.fontRenderer.setAntiAlias(true);
            ZWareMod.fontRenderer.setFractionalMetrics(true);
            ZWareMod.fontRenderer.setFontName(name);
    ...
    public void loadFriendsList() throws IOException {
        ...
        friendObject.forEach(object -> {
            ZWareMod.getInstance().friends.addFriend(object.getAsString());
        });
        inputStream.close();
    }
